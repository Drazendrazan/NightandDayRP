...
AddEventHandler("Framework:Client:OnJobUpdate", function(j)
    job = j.name
end)
...
function addBlipsForUnits()
    Framework.Functions.TriggerCallback("core_dispach:getUnits", function(units)
...
function openDispach()
    if Config.JobOne.job == job or Config.JobTwo.job == job or Config.JobThree.job == job then
        ...
        Framework.Functions.TriggerCallback( "core_dispach:getInfo", function(units, calls, ustatus)
