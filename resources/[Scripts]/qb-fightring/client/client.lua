...
RegisterNetEvent('qb-fight:playerJoined')
AddEventHandler('qb-fight:playerJoined', function(side, id)
...
RegisterNetEvent('qb-fight:startFight')
AddEventHandler('qb-fight:startFight', function(fightData)
...
RegisterNetEvent('qb-fight:playerLeaveFight')
AddEventHandler('qb-fight:playerLeaveFight', function(id)
    ...
    elseif participando == true then
        TriggerServerEvent('qb-fight:pay', betAmount)	
...
RegisterNetEvent('qb-fight:fightFinished')
AddEventHandler('qb-fight:fightFinished', function(looser)
    ...
    if participando == true then
        if(looser ~= GetPlayerServerId(PlayerId()) and looser ~= -2) then
            ...
            TriggerServerEvent('qb-fight:showWinner', GetPlayerServerId(PlayerId()))
        end
...
RegisterNetEvent('qb-fight:raiseActualBet')
AddEventHandler('qb-fight:raiseActualBet', function()
...
RegisterNetEvent('qb-fight:winnerText')
AddEventHandler('qb-fight:winnerText', function(id)
    showWinner = true
    winner = id
    Citizen.Wait(5000)
    showWinner = false
       ...
       Framework.Functions.DrawText3D(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z +1.5, 'Fighters: ~r~' .. players .. '/2 \n ~w~Bet: ~r~'.. betAmount ..'$ ', 0.4)
        ...
        if blueJoined == false then
            ...
            if blueZone < Config.DISTANCE_INTERACTION then
               if IsControlJustReleased(0, Config.E_KEY) and participando == false then
                    TriggerServerEvent('qb-fight:join', betAmount, 0 )
                end
            end
        end
        ...
        if redJoined == false then
            ...
            if redZone < Config.DISTANCE_INTERACTION then
                if IsControlJustReleased(0, Config.E_KEY) and participando == false then
                    TriggerServerEvent('qb-fight:join', betAmount, 1)
                end
            end
        end
    end
    ...
    while true do
        ...
        elseif showCountDown == false and fightStatus == STATUS_STARTED then
            if GetEntityHealth(PlayerPedId()) < 150 then
                TriggerServerEvent('qb-fight:finishFight', GetPlayerServerId(PlayerId()))
                fightStatus = STATUS_INITIAL
            end
        end
        ...
        if participando == true then
            ...
            if get3DDistance(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z,coords.x,coords.y,coords.z) > Config.LEAVE_FIGHT_DISTANCE then
                TriggerServerEvent('qb-fight:leaveFight', GetPlayerServerId(PlayerId()))
            end
        end
