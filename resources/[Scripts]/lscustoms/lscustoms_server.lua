	...
	if not button.purchased then
		if button.price then -- check if button have price
			if Player.Functions.RemoveMoney("cash", button.price, "lscustoms-bought") then
				...
				TriggerEvent("kh-log:server:sendLog", Player.PlayerData.citizenid, "vehicleupgraded", {name=name, moneyType="cash", price=button.price, plate="unkown"})
				TriggerEvent("kh-log:server:CreateLog", "vehicleupgrades", "Upgrade bought", "green", "**"..GetPlayerName(src).."** has bought an upgrade ("..name..") for €" .. button.price)
			elseif bankBalance >= button.price then
				Player.Functions.RemoveMoney("bank", button.price, "lscustoms-bought")
				TriggerClientEvent("LSC:buttonSelected", source, name, button, true)
				TriggerEvent("kh-log:server:sendLog", Player.PlayerData.citizenid, "vehicleupgraded", {name=name, moneyType="bank", price=button.price, plate="unkown"})
				TriggerEvent("kh-log:server:CreateLog", "vehicleupgrades", "Upgrade bought", "green", "**"..GetPlayerName(src).."** has bought an upgrade ("..name..") for €" .. button.price)
			else
...
AddEventHandler("lscustoms:server:SaveVehicleProps", function(vehicleProps)
    ...
    if IsVehicleOwned(vehicleProps.plate) then
        Framework.Functions.ExecuteSql(false, "UPDATE `player_vehicles` SET `mods` = '"..json.encode(vehicleProps).."' WHERE `plate` = '"..vehicleProps.plate.."'")
    end
end)
...
Framework.Commands.Add("livery", "Livery", {{name="livery", help="Nummer van de livery"}}, true, function(source, args)
