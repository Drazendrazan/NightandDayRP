                ...
                if not IsInHouse then
                    if not Config.Houses[Currenthouse]['Owned'] then
                      if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z'], true) < 3.0) then
                        DrawText3D(Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z'], '~g~E~w~ - View Property')
                            ...
                            if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Houses[Currenthouse]['Garage']['X'], Config.Houses[Currenthouse]['Garage']['Y'], Config.Houses[Currenthouse]['Garage']['Z'], true) < 3.0) then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    ...
                                    if not IsPedInAnyVehicle(PlayerPedId()) then
                                      exports['qb-garages']:OpenHouseGarage(Currenthouse)
                                    else
                                        Framework.Functions.TriggerCallback('qb-materials:server:is:vehicle:owned', function(IsOwned)
                                            if IsOwned then
                                                exports['qb-garages']:SetVehicleInHouseGarage(Currenthouse)
                                            else
                                                Framework.Functions.Notify("This is no-ones vehicle..", "error")
                                            end
...
AddEventHandler('qb-housing:client:enter:house', function()
    ...
    elseif Config.Houses[Currenthouse]['Tier'] == 7 then
        Other = {maxweight = 1200000, slots = 45}
        Housing = exports['qb-interiors']:HouseTierSeven(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 8 then
        Other = {maxweight = 1200000, slots = 45}
        Housing = exports['qb-interiors']:HouseTierEight(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 9 then
        Other = {maxweight = 1200000, slots = 45}
        Housing = exports['qb-interiors']:HouseTierNine(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 10 then
        Other = {maxweight = 1200000, slots = 45}
        Housing = exports['qb-interiors']:HouseTierTen(CoordsTable)
    ...
    Citizen.SetTimeout(450, function()
        exports['qb-houseplants']:LoadHousePlants(Currenthouse)
    end)
end)

RegisterNetEvent('qb-housing:client:add:to:config')
AddEventHandler('qb-housing:client:add:to:config', function(Name, Owner, CoordsTable, Owned, Tier, Price, DoorLocked, KeyHolder, Label, Garage)
    Config.Houses[Name] = {
        ['Coords'] = CoordsTable,
        ['Owned'] = Owned,
        ['Owner'] = Owner,
...
RegisterNetEvent('qb-housing:client:set:garage')
AddEventHandler('qb-housing:client:set:garage', function(HouseId, Coords)
...
RegisterNetEvent('qb-housing:client:delete:successful')
AddEventHandler('qb-housing:client:delete:successful', function(HouseId)
    Currenthouse = nil
    Config.Houses[HouseId] = {}
end)

RegisterNetEvent('qb-housing:client:delete:house')
AddEventHandler('qb-housing:client:delete:house', function()
    if Currenthouse ~= nil then 
        TriggerServerEvent('qb-housing:server:detlete:house', Currenthouse)
    else
        Framework.Functions.Notify("No property nearby", "error")
    end
end)

RegisterNetEvent('qb-housing:client:add:garage')
AddEventHandler('qb-housing:client:add:garage', function()
    if Currenthouse ~= nil then 
        local PlayerCoords = GetEntityCoords(PlayerPedId())
        local PlayerHeading = GetEntityHeading(PlayerPedId())
        local CoordsTable = {['X'] = PlayerCoords.x, ['Y'] = PlayerCoords.y, ['Z'] = PlayerCoords.z, ['H'] = PlayerHeading}
        TriggerServerEvent('qb-housing:server:add:garage', Currenthouse, Config.Houses[Currenthouse]['Adres'], CoordsTable)
    else
        Framework.Functions.Notify("No property nearby", "error")
    end
end)
...
AddEventHandler('qb-housing:client:give:keys', function()
  ...
  if Player ~= -1 and Distance < 1.5 then  
    ...
    Framework.Functions.Notify("No one found", "error")
  end
end)
...
AddEventHandler('qb-housing:client:ringdoor', function(Player, HouseId)
    if Currenthouse == HouseId and IsInHouse then
        ...
        Framework.Functions.Notify("There is someone at the door..")
    end
end)
...
AddEventHandler('qb-housing:client:reset:house:door', function()
    if IsNearHouse then
        if not Config.Houses[Currenthouse]['Door-Lock'] then
            ...
            Framework.Functions.Notify("This door is already closed..", 'error')
        end
    else
        Framework.Functions.Notify("No house?!?", 'error')
    end
end)
...
AddEventHandler('qb-housing:client:breaking:door', function()
    if IsNearHouse then
        if Config.Houses[Currenthouse]['Door-Lock'] then
            RamAnimation(true)
            Framework.Functions.Progressbar("bonk-door", "Breaking door..", 15000, false, true, {
...
function LeaveHouse()
    ...
    exports['qb-interiors']:DespawnInterior(HouseData, function()
        ...
        exports['qb-houseplants']:UnloadHousePlants(Currenthouse)
        UnloadDecorations()
        Citizen.Wait(1000)
        IsInHouse = false
        Other = nil
    ...
    Framework.Functions.GetPlayerData(function(PlayerData)
      for k, v in pairs(Config.Houses) do
         if Config.Houses[k]['Owner'] == PlayerData.citizenid then
            ...
            AddTextComponentSubstringPlayerName(Config.Houses[k]['Adress'])
