...
function SendCurrentObjectData()
    ...
    elseif CurrentEntity['IsCar'] then
        if Framework.Functions.GetPlayerData().job.name == 'mechanic' then
            SendNUIMessage({
                action = 'SetData',
                specific = false,
                currentdata = Config.MechanicMenu,
            })
            HitData = true
        else
          SendNUIMessage({
            ...
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        ...
                        Framework.Functions.Notify('Ever hopped in a trunk of a motorcycle?', 'error', 2500)
                    end
                else
