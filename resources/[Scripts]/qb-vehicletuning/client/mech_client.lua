...
RegisterNetEvent('mechanic:client:RepairVehicle')
AddEventHandler('mechanic:client:RepairVehicle', function()
	local vehicle = Framework.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		--print('Mechanic Script MADE BY RDX#7028')
...
function RepairVehicle(vehicle)
	...
	Framework.Functions.Progressbar("repair_vehicle", "Repairing..", 10000, false, true, {
		...
		Framework.Functions.Notify("Vehicle engine partially repaired")
...
RegisterNetEvent('mechanic:client:AdvancedRepairVehicle')
AddEventHandler('mechanic:client:AdvancedRepairVehicle', function()
	local vehicle = Framework.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		--print('Mechanic Script MADE BY RDX#7028')
		local pos = GetEntityCoords(GetPlayerPed(-1))
...
function AdvancedRepairVehicle(vehicle)
	...
	Framework.Functions.Progressbar("repair_vehicle", "Repairing..", 14000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		Framework.Functions.Notify("Vehicle repaired")
...
-- RegisterNetEvent('mechanic:client:CleanVehicle')
-- AddEventHandler('mechanic:client:CleanVehicle', function()
-- 	local vehicle = Framework.Functions.GetClosestVehicle()
-- 	if vehicle ~= nil and vehicle ~= 0 then
-- 		--print('Mechanic Script MADE BY RDX#7028')
-- 		local pos = GetEntityCoords(GetPlayerPed(-1))
...
-- 	Framework.Functions.Progressbar("repair_vehicle", "Cleaning Vehicle...", 2000, false, true, {
-- 		disableMovement = true,
-- 		disableCarMovement = true,
-- 		disableMouse = false,
-- 		disableCombat = true,
