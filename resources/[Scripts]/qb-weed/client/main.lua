...
function CheckCoords()
	Citizen.CreateThread(function()
		while true do
			local coords = GetEntityCoords(PlayerPedId())
			print('Weed plant growing (Fixed_by_colonelx)')
			if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedField.coords, true) < 500 then
				SpawnWeedPlants()
			end
			Citizen.Wait(4 * 60000)------before 8*
		end
	end)
end
...
function ProcessWeed()  --joint process
	...
	TriggerServerEvent('qb-weed:processCannabissanto')
	...
	Framework.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
	 ...
	 TriggerServerEvent('qb-weed:processCannabis') -- Done
...
function ProcessWeedmarijuana() --marijuana process
	...
	local playerPed = PlayerPedId()
	TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
	TriggerServerEvent('qb-weed:processCannabisxDsanto')
	Framework.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
	 ...
	 TriggerServerEvent('qb-weed:processCannabisxD') -- Done
...
function SpawnWeedPlants()
	while spawnedWeeds < 25 do
		...
		Framework.Functions.SpawnLocalObject('prop_weed_01', weedCoords, function(obj)
			---Framework.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj)
			PlaceObjectOnGroundProperly(obj)
			FreezeEntityPosition(obj, true)
			print('WEED Plant FIXED BY Colonelx')
