...
RegisterNetEvent('qb-stores:client:open:custom:store')
AddEventHandler('qb-stores:client:open:custom:store', function(ProductName)
    local Shop = {label = ProductName, items = Config.Products[ProductName], slots = 30}
    TriggerServerEvent("qb-inventory:server:OpenInventory", "shop", "custom", Shop)
end)
