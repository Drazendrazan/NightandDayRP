...
AddEventHandler('qb-items:client:drink', function(ItemName, PropName)
 ...
 Citizen.SetTimeout(1000, function()
 	TriggerEvent('qb-assets:addprop:with:anim', PropName, 'amb@world_human_drinking@coffee@male@idle_a', "idle_c", 10000)
...
RegisterNetEvent('qb-items:client:drink:alcohol')
AddEventHandler('qb-items:client:drink:alcohol', function(ItemName, PropName)
...
AddEventHandler('qb-items:client:drink:slushy', function()
 ...
 Citizen.SetTimeout(1000, function()
 	TriggerEvent('qb-assets:addprop:with:anim', 'Cup', 'amb@world_human_drinking@coffee@male@idle_a', "idle_c", 10000)
 	Framework.Functions.Progressbar("drink", "Drinking..", 6000, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		...
		exports['qb-assets']:RemoveProp()
		 ...
		 --TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['slushy'], "add")
 	end)
 end)
end)
	...
	--TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['armor'], "remove")
     Framework.Functions.Progressbar("vest", "Putting on vest..", 1000, false, true, {
		...
		disableCombat = true,
    	 ...
    	 --TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['armor'], "add")
     end)
 else
...
AddEventHandler("qb-items:client:reset:armor", function()
    ...
    if currentVest ~= nil and currentVestTexture ~= nil then 
        Framework.Functions.Progressbar("remove-armor", "Taking off vest..", 2500, false, true, {
			...
			Framework.Functions.TriggerCallback('qb-items:server:giveitem', 'heavy-armor', 1)
			TriggerServerEvent('qb-hospital:server:save:health:armor', GetEntityHealth(PlayerPedId()), GetPedArmour(PlayerPedId()))
        end)
    else
...
RegisterNetEvent('qb-items:client:use:cigarette')
AddEventHandler('qb-items:client:use:cigarette', function()
  Citizen.SetTimeout(1000, function()
    Framework.Functions.Progressbar("smoke-cigarette", "Taking out a cigarette..", 4500, false, true, {
     ...
     disableCombat = true,
        ...
        TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["ciggy"], "remove")
        TriggerEvent('qb-items:client:smoke:effect')
...
RegisterNetEvent('qb-items:client:smoke:effect')
AddEventHandler('qb-items:client:smoke:effect', function()
  OnWeed = true
  Time = 15
  while OnWeed do
    if Time > 0 then
     Citizen.Wait(1000)
     Time = Time - 1
     TriggerServerEvent('qb-hud:server:remove:stress', math.random(1, 3))
    end
     if Time <= 0 then
      OnWeed = false
     end 
