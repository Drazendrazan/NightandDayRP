...
function LockPickRegister(RegisterId, IsAdvanced)
 ...
 if math.random(1,10) < 9 then
    ...
    TriggerServerEvent('qb-police:server:send:alert:store', GetEntityCoords(PlayerPedId()), StreetLabel, Config.Registers[RegisterId]['SafeKey'])
 end
...
function CrackSafe(SafeId, IsAdvanced)
    ...
    if math.random(1,100) < 40 then
        local StreetLabel = Framework.Functions.GetStreetLabel()
        TriggerServerEvent('qb-police:server:send:alert:store', GetEntityCoords(PlayerPedId()), StreetLabel, SafeId)
    end
