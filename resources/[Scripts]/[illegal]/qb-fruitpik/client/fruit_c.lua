...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
        ...
        if (GetDistanceBetweenCoords(coords, 354.81, 6517.49, 28.24, true) < 20.0)  then
            ...
            DrawText3D2(354.81, 6517.49, 28.24, "~y~[E] ~b~Take orange")
           
            DrawText3D2(347.45, 6517.02, 28.79, "~y~[E] ~b~Take orange")
          
            DrawText3D2(339.54, 6516.92, 28.96, "~y~[E] ~b~Take orange")
...
AddEventHandler('wiadro:postawfp', function()
    ...
    if maitem == false then
        Framework.Functions.SpawnObject(model, {
            x = x,
            y = y,
            z = z
        }, function(obj)
    ...
    while true do
        ...
        if (GetDistanceBetweenCoords(coords, 404.08, 6526.95, 27.71, true) < 5.0) then
			...
			if(GetDistanceBetweenCoords(coords, 404.08, 6526.95, 27.71, true) < 3.0) then
				DrawText3D2(404.08, 6526.95, 27.71, '~y~[E] ~b~Make orang juice')
                ...
                if (hasBagd and hasBagdfb) then
                    ...
                    Framework.Functions.Notify('You have no to orange o emptybottle .', 'error')
                end
                end
            end
        end
...
Citizen.CreateThread(function()
	...
	local s1d7 = false
	if dist <= 2.0 then
	DrawText3D2(sellX, sellY, sellZ+0.1, "~y~[E] ~b~Sell orange juice")
		...
		Framework.Functions.Notify('You have no orangejuice .', 'error')
		end
		end	
	end
...
Citizen.CreateThread(function()
	...
	DrawText3D2(sellXa, sellYa, sellZa+0.1, "~y~[E] ~b~Sell orange3")
		...
		Framework.Functions.Notify('You have no orange .', 'error')
		end
		end	
	end
   ...
   local ad = "anim@heists@box_carry@"
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
		...
		if ( IsEntityPlayingAnim( player, ad, anim, 8 ) ) then
            ...
            Framework.Functions.Notify('load the oranges into the vehicle .', 'success', 7000)
		end
	end
...
while true do
    ...
    if coordsVisible then
        ...
        Framework.Functions.Notify('You put the ORANGE in the vehicle', 'success') --success
...
function SellKurczaki2fb()
    ...
    ----
    if(count == 0) then
    Framework.Functions.Progressbar("search_register", "processing..", 5000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
...
--------------------------------------------------------blip
 local blips = {
    {title="Orange", colour=47, id=467, x = 354.81, y = 6517.49, z = 28.24},
    {title="Orange", colour=47, id=467, x = 347.45, y = 6517.02, z = 28.79},
    {title="Orange", colour=47, id=467, x = 339.54, y = 6516.92, z = 28.96},
    {title="Orange Process juice", colour=47, id=467, x = 404.26, y = 6526.37, z = 27.7},
    {title="Sell Orange", colour=47, id=467, x = 417.86, y = 6519.82, z = 27.71},
    {title="Sell juice", colour=47, id=467, x = 409.59, y =6453.49, z = 28.81}
}
