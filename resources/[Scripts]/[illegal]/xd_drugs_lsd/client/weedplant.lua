	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				Framework.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pickup acid', 0.5)
			end
			local hasBagd = false
			local s1d = false
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				Framework.Functions.Progressbar("search_register", "Picking up Acid..", 7500, false, true, {
					...
					TriggerServerEvent('xd_drugs_weed:pickedUpCannabis2d')
				end, function()
					ClearPedTasks(GetPlayerPed(-1))
				end) -- Cancel
				...
				Framework.Functions.Notify('You dont have enough empity Bottle .', 'error')
			end	
			end
		else
			Citizen.Wait(500)
...
--prop_barrel_01a  prop_plant_01a
		Framework.Functions.SpawnLocalObject('prop_barrel_01a', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if not isProcessing then
				Framework.Functions.DrawText3D(Config.CircleZones.WeedProcessing.coords.x, Config.CircleZones.WeedProcessing.coords.y, Config.CircleZones.WeedProcessing.coords.z, 'Press ~g~[ E ]~w~ to Process')
			end
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				if (hasWeed and hasBag) then
					Processweed3d()
				elseif (hasWeed) then
					Framework.Functions.Notify('You dont have enough plastic bags.', 'error')
				elseif (hasBag) then
					Framework.Functions.Notify('You dont have enough Bottle acid.', 'error')
				else
					Framework.Functions.Notify('You dont have enough Bottle acid or plastic bags.', 'error')
				end
			end
		else
			Citizen.Wait(500)
...
function Processweed3d()  -- simple animations to loop while process is taking place
	...
	Framework.Functions.Progressbar("search_register", "Trying to Process..", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	 ...
	 TriggerServerEvent('xd_drugs_weed:processweed2d') -- Done
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1 then
			...
			if not isProcessing2 then
				Framework.Functions.DrawText3D(Config.CircleZones.DrugDealer.coords.x, Config.CircleZones.DrugDealer.coords.y, Config.CircleZones.DrugDealer.coords.z, 'Press ~g~[ E ]~w~ to Sell')
			end
			...
			if IsControlJustReleased(0, 38) and not isProcessing2 then
				...
				elseif (hasWeed2) then
					Framework.Functions.Notify('You dont have lsd.', 'error')
				elseif (hasBag2) then
					Framework.Functions.Notify('You dont have lsd.', 'error')
				else
					Framework.Functions.Notify('You dont have elsd.', 'error')
				end
			end
		else
			Citizen.Wait(500)
...
function SellDrug3()  -- simple animations to loop while process is taking place
	...
	Framework.Functions.Progressbar("search_register", "Trying to Process..", 15000, false, true, {
	  ...
	  -- Done
		...
		ClearPedTasks(GetPlayerPed(-1))
		Citizen.Wait(3500)
		TriggerServerEvent('xd_drugs_weed:selld2d')
	end, function()
		ClearPedTasks(GetPlayerPed(-1))
	end) -- Cancel
