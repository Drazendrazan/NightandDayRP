...
RegisterNetEvent('qb-banking:achieve')
AddEventHandler('qb-banking:achieve', function()
...
RegisterNetEvent('qb-banking:fail')
AddEventHandler('qb-banking:fail', function()
    StopAnimTask(PlayerPedId(), "anim@heists@prison_heiststation@cop_reactions", "cop_b_idle", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
                           ...
                           Framework.Functions.TriggerCallback("qb-bankrobbery:server:get:status", function(Status)
                               if not Status then
                                   ...
                                   if ClosestBank ~= 6 then          
                                    ...
                                    TriggerEvent('qb:numbers:start', time, function(result)
                                        if result then
                                            TriggerEvent('qb-inventory:client:set:busy', false)
                                            TriggerServerEvent('qb-bankrobbery:server:set:open', ClosestBank, true)
                                             Citizen.SetTimeout(1250, function()
                                    ...
                                    exports['minigame-memoryminigame']:StartMinigame({
                                        success = 'qb-banking:achieve',
                                        fail = 'qb-banking:fail'
                                    })
                                end   
                               else
