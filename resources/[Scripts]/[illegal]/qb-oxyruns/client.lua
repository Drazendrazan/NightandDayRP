...
function CreateOxyVehicle()
	...
	exports['qb-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(oxyVehicle), true)

    while true do
    	Citizen.Wait(1)
    	 Framework.Functions.DrawText3D(carspawns[spawnpoint]["x"], carspawns[spawnpoint]["y"], carspawns[spawnpoint]["z"], "Your Delivery Car.")
...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			Framework.Functions.Notify('The delivery was on point, your GPS will be updated with the next drop off..', 'success')
		else
			Framework.Functions.Notify('The drop off failed..', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			Framework.Functions.Notify('You are close to the drop off..', 'success')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			Framework.Functions.Notify('You are no longer selling Oxy due to taking too long to drop off the package..', 'warning')
		end
		if dstcheck < 2.0 and pedCreated then

			local crds = GetEntityCoords(deliveryPed)
			Framework.Functions.DrawText3D(crds["x"],crds["y"],crds["z"], "[E] Hand over")  
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			--Framework.Functions.DrawText3D(pillWorker["x"],pillWorker["y"],pillWorker["z"], "") 
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				Framework.Functions.Notify('The dealer isnt giving you anymore locations due to the state of his car..', 'error')
			else
				if tasking then
			        Citizen.Wait(1500)
			    else
	    	...
	    	for i = 1, #drugLocs do
				...
				if dstcheck < 5.0 then
					...
					if job ~= "police" then
			    		...
			    		Framework.Functions.DrawText3D(drugLocs[i]["x"],drugLocs[i]["y"],drugLocs[i]["z"], "[E] $" .. price .. " offer to sell stolen goods (12).") 
				    	...
				    	if IsControlJustReleased(0,38) then
							...
							if daytime > 17 or daytime < 9 then
								Framework.Functions.Notify('Its too late - no ones buying shit..', 'error')
		            		else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true 
	Framework.Functions.Notify('A car has been left outside for you. Your GPS will be updated with locations soon..', 'success')
	CreateOxyVehicle()
end)
