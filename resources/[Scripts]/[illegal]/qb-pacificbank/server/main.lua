...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            ...
            TriggerClientEvent("qb-pacific:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('qb-pacific:server:set:trolly:state')
AddEventHandler('qb-pacific:server:set:trolly:state', function(TrollyNumber, bool)
 Config.Trollys[TrollyNumber]['Open-State'] = bool
 TriggerClientEvent('qb-pacific:client:set:trolly:state', -1, TrollyNumber, bool)
end)

Framework.Functions.CreateCallback("qb-pacific:server:get:status", function(source, cb)
    cb(IsBankBeingRobbed)
  end)

RegisterServerEvent('qb-pacific:server:Klapdebank')
AddEventHandler('qb-pacific:server:Klapdebank', function(state)
        if not robberyBusy then
			Config.PacificB["explosive"]["isOpened"] = state
            TriggerClientEvent('qb-pacific:client:Klapdebank', state)
            IsBankBeingRobbed = true
            TriggerEvent('qb-pacific:server:setTimeout')
        else
			Config.PacificB["explosive"]["isOpened"] = state
...
RegisterServerEvent('qb-pacific:server:set:lights')
AddEventHandler('qb-pacific:server:set:lights', function(bool)
 Config.PacificB['lights'] = bool
 TriggerClientEvent('qb-pacific:client:set:lights:state', -1, bool)
end)


RegisterServerEvent('qb-pacific:server:set:trollyz')
AddEventHandler('qb-pacific:server:set:trollyz', function(bool)
    Config.PacificB["isOpened"] = bool
 TriggerClientEvent('qb-pacific:client:set:trol:state', -1, bool)
end)


RegisterServerEvent('qb-pacific:server:cabinetItem')
AddEventHandler('qb-pacific:server:cabinetItem', function(type)
    local src = source
    local ply = Framework.Functions.GetPlayer(src)
        ...
        if tierChance < 25 then tier = 1 elseif tierChance >= 25 and tierChance < 50 then tier = 2 elseif tierChance >= 50 and tierChance < 75 then tier = 3 elseif tierChance >=75 and tierChance <85 then tier = 4 end
            if tier ~= 4 then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['pistol-ammo'], "add")
            end   
    end)

RegisterServerEvent('qb-pacific:server:recieveItem')
AddEventHandler('qb-pacific:server:recieveItem', function(type)
    local src = source
    local ply = Framework.Functions.GetPlayer(src)
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', ply.PlayerData.source, Framework.Shared.Items[item.item], 'add')
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, ply.PlayerData.source, Framework.Shared.Items['money-roll'], "add")
            end
        else
...
Framework.Functions.CreateCallback('qb-pacific:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

Framework.Functions.CreateCallback('qb-pacific:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('qb-pacific:server:setTimeout')
AddEventHandler('qb-pacific:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                TriggerClientEvent('qb-pacific:client:ClearTimeoutDoors', -1)
            end)
        end
    end
end)

Framework.Functions.CreateCallback('qb-pacific:server:PoliceAlertMessage', function(source, cb, title, coords, blip)
	local src = source
    local alertData = {
        title = title,
            ...
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                if blip then
                    if not alarmTriggered then
                        ...
                        TriggerClientEvent("qb-pacific:client:PoliceAlertMessage", v, title, coords, blip)
                        alarmTriggered = true
                    end
                else
                    TriggerClientEvent("qb-phone:client:addPoliceAlert", v, alertData)
...
RegisterServerEvent('qb-pacific:server:SetStationStatus')
AddEventHandler('qb-pacific:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("qb-pacific:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("qb-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("qb-pacific:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
Framework.Commands.Add("bankreset", "Bank reset", {}, false, function(source, args)
    ...
    if group == "god"  then
        TriggerEvent('qb-pacific:server:setTimeout', source)
	end
end)

Framework.Functions.CreateCallback('qb-pacificbank:server:HasItem', function(source, cb, ItemName)
...
RegisterServerEvent('qb-pacific:server:rob:pacific:money')
AddEventHandler('qb-pacific:server:rob:pacific:money', function()
  local Player = Framework.Functions.GetPlayer(source)
...
RegisterServerEvent('qb-pacific:maze:server:DoSmokePfx')
AddEventHandler('qb-pacific:maze:server:DoSmokePfx', function()
    TriggerClientEvent('qb-pacific:maze:client:DoSmokePfx', -1)
end)
...
Framework.Functions.CreateUseableItem("black-card", function(source, item)
	local Player = Framework.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('qb-pacificbank:client:use:black-card', source, 'purple-card')
    end
end)
