...
local laser2 =
    ...
    {travelTimeBetweenTargets = {1.0, 1.0}, waitTimeAtTargets = {0.0, 0.0}, name = "QB2"}
)
                ...
                if Config.PacificB["lights"] then
                    ...
                    if Area < 2.0 then
                        ...
                        if not AlarmItem then
                            AlarmItem = true
                            TriggerEvent("qb-inventory:client:requiredItems", Alarm, true)
                        end
                    else
                        if AlarmItem then
                            AlarmItem = false
                            TriggerEvent("qb-inventory:client:requiredItems", Alarm, false)
                        end
                    end
                else
...
RegisterNetEvent("qb-pacific:client:PoliceAlertMessage")
AddEventHandler(
    "qb-pacific:client:PoliceAlertMessage",
...
RegisterNetEvent("qb-pacific:client:Klapdebank")
AddEventHandler(
    "qb-pacific:client:Klapdebank",
    function(state)
...
--Citizen.CreateThread(
   -- function()
     --   Citizen.Wait(18000)
     --   TriggerServerEvent("qb:licenseCheck")
   -- end
--)

RegisterNetEvent("qb-pacific:client:setfirstState")
AddEventHandler(
    "qb-pacific:client:setfirstState",
    function(state)
        Config.PacificB["explosive"]["isOpened"] = state
        TriggerServerEvent("qb-pacific:server:setTimeout")
    end
)

RegisterNetEvent("qb-pacific:client:setLockerState")
AddEventHandler(
    "qb-pacific:client:setLockerState",
    function(lockerId, state, bool)
        Config.PacificB["lockers"][lockerId][state] = bool
    end
)

RegisterNetEvent("qb-pacific:client:set:lights:state")
AddEventHandler(
    "qb-pacific:client:set:lights:state",
    function(bool)
        Config.PacificB["lights"] = bool
    end
)

RegisterNetEvent("qb-pacific:client:set:trol:state")
AddEventHandler(
    "qb-pacific:client:set:trol:state",
    function(state)
...
RegisterNetEvent("qb-pacific:client:DoSmokePfx")
AddEventHandler(
    "qb-pacific:client:DoSmokePfx",
    function()
...
RegisterNetEvent("qb-pacific:client:clear:trollys")
AddEventHandler(
    "qb-pacific:client:clear:trollys",
    function()
        Count = 0
...
RegisterNetEvent("qb-pacific:client:set:trolly:state")
AddEventHandler(
    "qb-pacific:client:set:trolly:state",
