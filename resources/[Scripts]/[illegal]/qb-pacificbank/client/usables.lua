                                ...
                                exports["minigame-phone"]:StartHack(
                                    ...
                                    function(Success)
                                        if Success then
                                            ...
                                            TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["electronickit"], "remove")

                                            TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["trojan_usb"], "remove")

                                            TriggerEvent("qb-inventory:client:set:busy", false)

                                            TriggerServerEvent("qb-doorlock:server:updateState", 36, false)

                                            TriggerServerEvent("qb-pacific:server:set:trollyz", true)
                                        else
    ...
    function()
        ...
        if CurrentCops >= Config.PacificBPolice then
            if ExplosiveRange then
                Framework.Functions.TriggerCallback(
                    "qb-pacific:server:isRobberyActive",
                    function(isBusy)
                        if not isBusy then
                            ...
                            TriggerServerEvent("qb-pacific:server:DoSmokePfx")
                            ...
                            deuren = true
                            deuropen = true
                            TriggerServerEvent("qb-doorlock:server:updateState", 35, false)
                            TriggerServerEvent("qb-pacific:server:Klapdebank", true)
                            Framework.Functions.TriggerCallback(
                                "qb-pacific:server:PoliceAlertMessage",
                                function(result)
                                end,
                                "Pacific Bank",
                                Positie,
...
RegisterNetEvent("qb-pacificbank:client:use:black-card")
AddEventHandler(
    "qb-pacificbank:client:use:black-card",
    function()
                                        ...
                                        function(Success)
                                            if Success then
                                                TriggerEvent(
                                                    ...
                                                    function(Outcome)
                                                        if Outcome then
                                                            TriggerServerEvent("qb-pacific:server:set:lights", false)
                                                            TriggerServerEvent(
