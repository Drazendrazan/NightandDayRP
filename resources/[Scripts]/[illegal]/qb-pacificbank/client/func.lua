function LockDownStart()
    ...
    if not copsCalled then
        if Config.PacificB["alarm"] then
            --TriggerServerEvent("qb-pacific:server:callCops", GetEntityCoords(PlayerPedId()))
            copsCalled = true
        end
    end
    -- SpawnSecurity()
...
function GetMoneyFromTrolly(TrollyNumber)
    ...
    GrabbingMoney = false
    TriggerServerEvent("qb-pacific:server:set:trolly:state", TrollyNumber, true)
    TriggerServerEvent("qb-pacific:server:rob:pacific:money")
