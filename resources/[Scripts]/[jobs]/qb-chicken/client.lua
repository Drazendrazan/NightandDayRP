...
if share == true then
	...
	andsplashed = 0
	share = false
	Framework.Functions.Notify("Take Alived Chiken To Process Area ..", "primary")
...
function hewassplashed()
	...
	if chanceofsplashes <= 60 then
			Framework.Functions.Notify("You managed to catch 1 chicken!", "success")
			if Caught1 == 1 then
		...
		Framework.Functions.Notify("The chicken escaped your arms!", "error")
	end
end
...
function packingg(stanowisko)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			Framework.Functions.Progressbar("wash-", "Packing..", 30000, false, true, {
			...
			Framework.Functions.Notify("Keep packing the chicken or go to the vehicle and store it.", "primary")
		...
		Framework.Functions.Notify("You have nothing to pack!", "error")
		end
...
function packed(stanowisko)
	...
	LoadDict('anim@heists@box_carry@')
		...
		Framework.Functions.Notify("You stopped packing!", "error")
...
function portioning(position)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			Framework.Functions.Progressbar("Cut-", "Slaughtering..", 25000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			Framework.Functions.Notify("Now Pack slaughtered chicken!", "primary")
			...
			Framework.Functions.Notify("You dont have any chickens!", "error")
		end
	end, 'alivechicken')
end
...
function packedsell()
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			Framework.Functions.Progressbar("Cut-", "Selling..", 10000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			Framework.Functions.Notify("You have nothing to sell!", "error")
		end
