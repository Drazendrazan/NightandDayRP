...
Framework.Commands.Add("resetmilage", "Reset vehicle milage", {{name="registration", help="Registration number of vehicle."}}, false, function(source, args)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
			...
			TriggerClientEvent('qb-vehicletuning:client:ResetDrivingDistance', -1, VehicleDrivingDistance[kenteken], kenteken)
			...
			Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_vehicles` WHERE `plate` = '"..Framework.CleanseString(kenteken).."'", function(result)
				...
				TriggerClientEvent('Framework:Notify', source, "Registration number of vehicle not correct!")
				end
			end)
        else
            TriggerClientEvent('Framework:Notify', source, "No Registration number provided.")
        end
    else
        TriggerClientEvent('Framework:Notify', source, "You can not do this!", "error") 
    end
end)
...
Framework.Commands.Add("setvehiclestatus", "Set vehicle status", {{name="part", help="Part you want to work on"}, {name="amount", help="Status level"}}, true, function(source, args)
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            ...
            for i = 1, Config.RepairCostAmount[part].costs, 1 do
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items[Config.RepairCostAmount[part].item], "remove")
                Citizen.Wait(500)
            end
        else
            TriggerClientEvent('Framework:Notify', src, "You do not have enough "..Framework.Shared.Items[Config.RepairCostAmount[part].item]["label"].." (min. "..Config.RepairCostAmount[part].costs.."x)", "error")
        end
    else
        TriggerClientEvent('Framework:Notify', src, "You have no "..Framework.Shared.Items[Config.RepairCostAmount[part].item]["label"].." with you!", "error")
    end
end)
...
Framework.Commands.Add("setmechanic", "Hire a mechanic", {{name="id", help="CID"}}, false, function(source, args)
    local Player = Framework.Functions.GetPlayer(source)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('Framework:Notify', TargetData.PlayerData.source, "You were hired to autocare!")
                TriggerClientEvent('Framework:Notify', source, "You have hired ("..TargetData.PlayerData.charinfo.firstname..") as Autocare medewerker!")
            end
        else
            TriggerClientEvent('Framework:Notify', source, "Provide a player ID!")
        end
    else
        TriggerClientEvent('Framework:Notify', source, "You can not do this!", "error") 
    end
...
Framework.Commands.Add("firemechanic", "Fire a mechanic", {{name="id", help="CID"}}, false, function(source, args)
    local Player = Framework.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('Framework:Notify', TargetData.PlayerData.source, "You were fired from Autocare!")
                    TriggerClientEvent('Framework:Notify', source, "You fired ("..TargetData.PlayerData.charinfo.firstname..") from autocare!")
                else
                    TriggerClientEvent('Framework:Notify', source, "This is not a employee of autocare!", "error")
                end
            end
        else
            TriggerClientEvent('Framework:Notify', source, "Provide a player ID!", "error")
        end
    else
        TriggerClientEvent('Framework:Notify', source, "You can not do this!", "error")
    end
