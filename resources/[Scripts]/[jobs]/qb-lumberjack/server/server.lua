...
AddEventHandler('wood_weed:processweed2', function()
	...
	if Player.Functions.GetItemByName('wood_cut') then
		...
		if chance == 1 or chance == 2 or chance == 3 or chance == 4 or chance == 5 or chance == 6 or chance == 7 or chance == 8 then
			...
			TriggerClientEvent("qb-inventory:client:ItemBox", source, Framework.Shared.Items['wood_cut'], "remove")
			TriggerClientEvent("qb-inventory:client:ItemBox", source, Framework.Shared.Items['wood_proc'], "add")
	...
	if Item == nil then
	 ...
	 for k, v in pairs(Config.Prices) do
		...
		if Item.amount > 0 then
			...
			TriggerClientEvent("qb-inventory:client:ItemBox", source, Framework.Shared.Items['wood_proc'], "remove")
