...
AddEventHandler('qb-tow:server:recieve:payment', function()
    ...
    if Player.Functions.AddMoney('cash', Config.JobData[Player.PlayerData.citizenid]['Payment']) then
      local AmountNetto = Config.JobData[Player.PlayerData.citizenid]['Payment'] + math.random(1250, 2000)
      TriggerClientEvent('chatMessage', source, "Towing LS", "warning", "You recieved your payslip: $"..AmountNetto..", untaxed: $"..Config.JobData[Player.PlayerData.citizenid]['Payment'])
