...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for i = 1, #QBCustoms.ShowroomPositions, 1 do
        local oldVehicle = GetClosestVehicle(QBCustoms.ShowroomPositions[i].coords.x, QBCustoms.ShowroomPositions[i].coords.y, QBCustoms.ShowroomPositions[i].coords.z, 3.0, 0, 70)
        if oldVehicle ~= 0 then
            Framework.Functions.DeleteVehicle(oldVehicle)
        end

		local model = GetHashKey(QBCustoms.ShowroomPositions[i].vehicle)
		RequestModel(model)
		while not HasModelLoaded(model) do
			Citizen.Wait(0)
		end

		local veh = CreateVehicle(model, QBCustoms.ShowroomPositions[i].coords.x, QBCustoms.ShowroomPositions[i].coords.y, QBCustoms.ShowroomPositions[i].coords.z, false, false)
		SetModelAsNoLongerNeeded(model)
		SetVehicleOnGroundProperly(veh)
		SetEntityInvincible(veh,true)
        SetEntityHeading(veh, QBCustoms.ShowroomPositions[i].coords.h)
...
function SetClosestCustomVehicle()
    ...
    for id, veh in pairs(QBCustoms.ShowroomPositions) do
        if current ~= nil then
            if(GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[id].coords.x, QBCustoms.ShowroomPositions[id].coords.y, QBCustoms.ShowroomPositions[id].coords.z, true) < dist)then
                current = id
                dist = GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[id].coords.x, QBCustoms.ShowroomPositions[id].coords.y, QBCustoms.ShowroomPositions[id].coords.z, true)
            end
        else
            dist = GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[id].coords.x, QBCustoms.ShowroomPositions[id].coords.y, QBCustoms.ShowroomPositions[id].coords.z, true)
            current = id
...
Citizen.CreateThread(function()
    while true do
        local pos = GetEntityCoords(GetPlayerPed(-1), true)
        local ShopDistance = GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[1].coords.x, QBCustoms.ShowroomPositions[1].coords.y, QBCustoms.ShowroomPositions[1].coords.z, false)
...
Citizen.CreateThread(function()
    while true do
        local ped = GetPlayerPed(-1)
        local pos = GetEntityCoords(ped, false)
        local dist = GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z, false)

        if isLoggedIn then
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" then
                        DrawText3Ds(QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9, '~g~G~w~ - Change vehicle')
                        DrawText3Ds(QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.75, '~b~/sellvehicle [id]~w~ -Sell vehicle ~b~/Testdrive~w~ - Arrange test drive')
                        ...
                        if ClosestCustomVehicle ~= nil then
                            if QBCustoms.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if QBCustoms.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if Framework.Shared.Vehicles[QBCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        DrawText3Ds(QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - Kopen / ~r~8~w~ - Annuleren - ~g~(€'..Framework.Shared.Vehicles[QBCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        
                                        if IsDisabledControlJustPressed(0, Keys["7"]) then
                                            TriggerServerEvent('qb-vehicleshop:server:ConfirmVehicle', QBCustoms.ShowroomPositions[ClosestCustomVehicle])
                                            QBCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end

                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            Framework.Functions.Notify('You did not buy the vehicle!', "error")
                                            QBCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
AddEventHandler('qb-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    CancelEvent()
    if QBCustoms.ShowroomPositions[k].vehicle ~= showroomVehicle then
        Framework.Functions.DeleteVehicle(GetClosestVehicle(QBCustoms.ShowroomPositions[k].coords.x, QBCustoms.ShowroomPositions[k].coords.y, QBCustoms.ShowroomPositions[k].coords.z, 3.0, 0, 70))
        ...
        local veh = CreateVehicle(model, QBCustoms.ShowroomPositions[k].coords.x, QBCustoms.ShowroomPositions[k].coords.y, QBCustoms.ShowroomPositions[k].coords.z, false, false)
        SetModelAsNoLongerNeeded(model)
        SetVehicleOnGroundProperly(veh)
        SetEntityInvincible(veh,true)
        SetEntityHeading(veh, QBCustoms.ShowroomPositions[k].coords.h)
        SetVehicleDoorsLocked(veh, 3)

        FreezeEntityPosition(veh, true)
        SetVehicleNumberPlateText(veh, k .. "CARSALE")
        CustomModelLoaded =  true
        QBCustoms.ShowroomPositions[k].vehicle = showroomVehicle
    end
end)
...
AddEventHandler('qb-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
    Framework.Functions.SpawnVehicle(Showroom.vehicle, function(veh)
        ...
        SetEntityHeading(veh, QBCustoms.VehicleBuyLocation.h)
    ...
    end, QBCustoms.VehicleBuyLocation, false)
end)
...
AddEventHandler('qb-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        Framework.Functions.SpawnVehicle(QBCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['qb-fuel']:SetFuelLevel(veh, GetVehicleNumberPlateText(veh), 100, false)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
            ...
            if QBCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle == "urus" then
                SetVehicleExtra(veh, 1, false)
                SetVehicleExtra(veh, 2, true)
            end
        end, QBCustoms.VehicleBuyLocation, false)
...
AddEventHandler('qb-vehicleshop:client:SellCustomVehicle', function(TargetId)
    ...
    if player ~= -1 and distance < 2.5 then
        local VehicleDist = GetDistanceBetweenCoords(pos, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z)

        if VehicleDist < 2.5 then
            TriggerServerEvent('qb-vehicleshop:server:SellCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            Framework.Functions.Notify("You are not close to a vehicle!", "error")
        end
    else
...
AddEventHandler('qb-vehicleshop:client:SetVehicleBuying', function(slot)
    QBCustoms.ShowroomPositions[slot].buying = true
    SetTimeout((60 * 1000) * 5, function()
        QBCustoms.ShowroomPositions[slot].buying = false
    end)
end)
