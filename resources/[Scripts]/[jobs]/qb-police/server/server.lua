...
Framework.Commands.Add("unjail", "Unjail a person.", {{name="id", help="Player ID"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent("qb-prison:client:leave:prison", playerId)
    else
    ...
    if TargetPlayer ~= nil then
       if Player.PlayerData.job.name == "police" then
         if Amount > 0 then
          ...
          TriggerEvent("qb-bossmenu:server:addAccountMoney", "police", 0.85 * Amount)
       ...
       elseif Player.PlayerData.job.name == "realestate" then
        if Amount > 0 then
               TriggerEvent('qb-phone:server:add:invoice', TargetPlayer.PlayerData.citizenid, Amount, 'Real Estate', 'realestate')  
           else
               TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "The amount must be greater than 0")
           end
       else
    ...
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("qb-inventory:client:ItemBox", source, Framework.Shared.Items["filled_evidence_bag"], "add")
            end
        else
...
RegisterServerEvent('qb-police:server:SyncSpikes')
AddEventHandler('qb-police:server:SyncSpikes', function(table)
    TriggerClientEvent('qb-police:client:SyncSpikes', -1, table)
end)