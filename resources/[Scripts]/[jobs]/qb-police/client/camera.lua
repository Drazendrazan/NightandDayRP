...
RegisterNetEvent('qb-police:client:CameraCommand')
AddEventHandler('qb-police:client:CameraCommand', function(cameraId)
    local plyPed = PlayerPedId()
    local plyCoords = GetEntityCoords(plyPed)
    local distance = GetDistanceBetweenCoords(plyCoords.x, plyCoords.y,plyCoords.z,438.75601, -992.4405, 30.689327, true)

    --if distance < 4 then
        TriggerEvent('qb-police:client:ActiveCamera', cameraId)
    --end
end)

RegisterNetEvent('qb-police:client:ActiveCamera')
AddEventHandler('qb-police:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        KCore.Functions.Notify("Camera does not exist..", "error")
    end
end)

RegisterNetEvent('qb-police:client:DisableAllCameras')
AddEventHandler('qb-police:client:DisableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('qb-police:client:EnableAllCameras')
AddEventHandler('qb-police:client:EnableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('qb-police:client:SetCamera')
AddEventHandler('qb-police:client:SetCamera', function(key, isOnline)
