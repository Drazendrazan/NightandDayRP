...
Citizen.CreateThread(function()
    for k, v in pairs(QB.VehicleShops) do
        ...
        AddTextComponentSubstringPlayerName("Imported Cars")
                        ...
                        if ClosestCustomVehicle ~= nil then
                            if QBCustom.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if QBCustom.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if Framework.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        DrawText3Ds(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - Buy / ~r~8~w~ - Cancel - ~g~(€'..Framework.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        ...
                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            Framework.Functions.Notify('You did NOT buy the car!')
                                            QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
AddEventHandler('qb-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    ...
    if QBCustom.ShowroomPositions[k].vehicle ~= showroomVehicle then
        Framework.Functions.DeleteVehicle(GetClosestVehicle(QBCustom.ShowroomPositions[k].coords.x, QBCustom.ShowroomPositions[k].coords.y, QBCustom.ShowroomPositions[k].coords.z, 3.0, 0, 70))
...
AddEventHandler('qb-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        Framework.Functions.SpawnVehicle(QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['qb-fuel']:SetFuelLevel(veh, GetVehicleNumberPlateText(veh), 100.0, false)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
...
AddEventHandler('qb-vehicleshop:client:SellCustomVehicle', function(TargetId)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            ...
            Framework.Functions.Notify("You are not near the vehicle!", "error")
        end
    else
        Framework.Functions.Notify("No one around!", "error")
    end
end)
