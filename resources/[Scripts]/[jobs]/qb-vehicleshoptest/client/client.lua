...
RegisterNetEvent('qb-vehicleshoptest.receiveInfo')
AddEventHandler('qb-vehicleshoptest.receiveInfo', function(bank, name)
    if name then
        profileName = name
    end
    profileMoney = bank
...
RegisterNetEvent('qb-vehicleshoptest.successfulbuy')
AddEventHandler('qb-vehicleshoptest.successfulbuy', function(vehicleName,vehiclePlate,value)    
    SendNUIMessage(
        {
            type = "successful-buy",
...
RegisterNetEvent('qb-vehicleshoptest.notify')
AddEventHandler('qb-vehicleshoptest.notify', function(type, message)    
    SendNUIMessage(
        {
            type = "notify",
            typenotify = type,
...
RegisterNetEvent('qb-vehicleshoptest.vehiclesInfos')
AddEventHandler('qb-vehicleshoptest.vehiclesInfos', function() 
...
function OpenVehicleShop()
    ...
    TriggerServerEvent("qb-vehicleshoptest.requestInfo")
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        local vehicleProps = Framework.Functions.GetVehicleProperties(lastSelectedVehicleEntity)
        vehicleProps.plate = newPlate

        TriggerServerEvent('qb-vehicleshoptest.CheckMoneyForVeh',data.modelcar, data.sale, data.name, vehicleProps)
...
RegisterNetEvent('qb-vehicleshoptest.spawnVehicle')
AddEventHandler('qb-vehicleshoptest.spawnVehicle', function(model, plate)    
    local hash = GetHashKey(model)
    ...
    exports['qb-fuel']:SetFuelLevel(vehicleBuy, GetVehicleNumberPlateText(vehicleBuy), 100, false)
    SetPedIntoVehicle(PlayerPedId(), vehicleBuy, -1)
    SetVehicleNumberPlateText(vehicleBuy, plate)
    exports['qb-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(vehicleBuy), true)
...
RegisterNUICallback(
    ...
    function(data, cb)        
        if Config.TestDrive then
            ...
            exports['qb-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(testDriveEntity), true)
