...
AddEventHandler('vehiclefailure:client:RepairVehicle', function()
	...
	if vehicle ~= nil and vehicle ~= 0 then
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			Framework.Functions.Notify("Something Is Wrong!", "error")
		end
	end
end)
	...
	if vehicle ~= nil and vehicle ~= 0 then
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, drawpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
				    ...
				    local time = math.random(60000, 90000)
				    playAnim("mini@repair", "fixing_a_ped", time)
					Framework.Functions.Progressbar("repair_vehicle", "Repairing Vehicle..", time, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
					}, {}, {}, {}, function() -- Done
						Framework.Functions.Notify("Vehicle Repaired, get to a mechanic to polish her up!")
...
function CleanVehicle(vehicle)
	...
	Framework.Functions.Progressbar("repair_vehicle", "Cleaning...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		Framework.Functions.Notify("Vehicle cleaned!")
...
RegisterNetEvent('vehiclefailure:client:CleanVehicleMechanic')
AddEventHandler('vehiclefailure:client:CleanVehicleMechanic', function()
	local vehicle = Framework.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local pos = GetEntityCoords(GetPlayerPed(-1))
		local vehpos = GetEntityCoords(vehicle)
...
RegisterNetEvent('vehiclefailure:client:RepairVehicleFullMechanic')
AddEventHandler('vehiclefailure:client:RepairVehicleFullMechanic', function()
	local vehicle = Framework.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local pos = GetEntityCoords(GetPlayerPed(-1))
		local vehpos = GetEntityCoords(vehicle)
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, drawpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
				    ...
				    local ninja = math.random(200000, 240000)
				    playAnim("mini@repair", "fixing_a_ped", ninja)
					Framework.Functions.Progressbar("repair_vehicle", "Repairing Vehicle..", ninja, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
					}, {}, {}, {}, function() -- Done
						Framework.Functions.Notify("Fully Fixed!")
...
function RepairVehicleFull(vehicle)
    ...
    local time = math.random(100000, 180000)
    playAnim("mini@repair", "fixing_a_ped", time)
	Framework.Functions.Progressbar("repair_vehicle", "Repairing Vehicle..", time, false, true, {
		disableMovement = true,
		disableCarMovement = true,
