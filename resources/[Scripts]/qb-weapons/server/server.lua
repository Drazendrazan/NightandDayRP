...
Framework.Commands.Add("addammo", "Give bullets for the weapon you are holding", {{name="Amount", help="Amount of bullets?"}}, true, function(source, args)
    if args[1] ~= nil then
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("Framework:Notify", src, "You already have a "..AttachmentData.label:lower().." on your weapon.", "error", 3500)
            end
        else
