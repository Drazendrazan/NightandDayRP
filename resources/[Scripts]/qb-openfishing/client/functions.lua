TryToFish = function()
    Framework.Functions.TriggerCallback('qb-openfishing:GetItemData', function(count)
...
CastBait = function(rodHandle, castLocation)
	...
	TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["fishingbait"], "remove")
...
function HasFishingBait()
    local rtval = false
    Framework.Functions.TriggerCallback('Framework:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
