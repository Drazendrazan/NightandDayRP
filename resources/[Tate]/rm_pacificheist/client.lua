...
AddEventHandler('pacificheist:client:policeAlert', function(targetCoords)
	Framework.Functions.Notify(Strings['police_alert'], "error")
    local alpha = 250
...
function StartHeist()
    Framework.Functions.TriggerCallback('pacificheist:server:checkPoliceCount', function(status)
        if status then
            Framework.Functions.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
					Framework.Functions.Notify(Strings['start_heist'], "error")
					Framework.Functions.Notify(Strings['start_heist2'], "error")
...
function LaptopHack(index)
    Framework.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem)
        if hasItem then
			...
			Framework.Functions.Notify(Strings['need_this'], "error")
        end
...
function OverheatScene()
    Framework.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
...
function PaintingScene(sceneId)
    Framework.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem)
        if hasItem then
            ...
            if weapon ~= GetHashKey('WEAPON_SWITCHBLADE') then ShowNotification(Strings['need_switchblade']) Framework.Functions.Notify(Strings['need_switchblade'], "error") return end
...
function Outside(index)
    if robber then
        robber = false
		Framework.Functions.Notify(Strings['deliver_to_buyer'], "primary")
        loadModel('baller')
