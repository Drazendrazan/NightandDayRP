...
local start = false
discord = {
    ['webhook'] = 'https://discord.com/api/webhooks/958264305084690432/jMfsDYjstiVwnpJ_FdXs9wjyzUWPPv0LUKdEZh7a_VeMWn0dvDuQutbcfsNxa1VojsmV',
...
Framework.Functions.CreateCallback('pacificheist:server:checkPoliceCount', function(source, cb)
    local src = source
...
Framework.Functions.CreateCallback('pacificheist:server:checkTime', function(source, cb)
    local src = source
...
Framework.Functions.CreateCallback('pacificheist:server:hasItem', function(source, cb, item)
    local src = source
    local player = Framework.Functions.GetPlayer(src)
...
AddEventHandler('pacificheist:server:rewardItem', function(item, count, type)
    ...
    if player then
        if type == 'money' then
            ...
            if dist > 200.0 then
                TriggerClientEvent('Framework:Notify', src, "Add money exploit playerID: " .. src .. 'name: ' .. player.PlayerData.name, "error")
            else
                if Config['PacificHeist']['black_money'] then
                    player.Functions.AddItem('bands', count)
					TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items['bands'], "add")
                else
            ...
            if whitelistItems[item] then
                ...
                TriggerClientEvent('Framework:Notify', src, "Add item exploit playerID: " .. src .. 'name: ' .. player.PlayerData.name, "error")
            end
        end
    end
end)
...
AddEventHandler('pacificheist:server:removeItem', function(item)
    ...
    if player then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items[item], "remove")
    end
end)
...
AddEventHandler('pacificheist:server:sellRewardItems', function()
    ...
    if player then
        ...
        TriggerClientEvent('Framework:Notify', src, Strings['total_money'] .. ' $' .. math.floor(totalMoney), "success")
    end
end)
...
RegisterCommand('pdpacific', function(source, args)
    ...
    if player then
        if (player.PlayerData.job.name == "police" and player.PlayerData.job.onduty) then
            ...
            TriggerClientEvent('Framework:Notify', src, "You are not cop!", "error")
        end
    end
end)
-- Leaked By: Leaking Hub | J. Snow | https://leakinghub.epizy.com/index.php
