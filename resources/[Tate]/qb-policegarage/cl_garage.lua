...
AddEventHandler('onClientResourceStart',function()
    Citizen.CreateThread(function()
        while true do
            if Framework ~= nil and Framework.Functions.GetPlayerData ~= nil then
                Framework.Functions.GetPlayerData(function(PlayerData)
...
CreateThread(function()
    while true do
        Citizen.Wait(3)
        if PlayerJob.name == "police" and PlayerJob.grade >= 2 then
            local inRange = false
...
RegisterNUICallback("showVeh", function(data,cb)
    -- CLEAR SPACE
    local vehinarea = Framework.Functions.GetVehiclesInArea(vector3(452.06686, -1023.903, 28.76457), 1)
    if #vehinarea ~= 0 then
        Framework.Functions.DeleteVehicle(vehinarea[1])
    end

    -- SPAWN VEHICLE
    Framework.Functions.SpawnVehicle(data.model, function(veh)
