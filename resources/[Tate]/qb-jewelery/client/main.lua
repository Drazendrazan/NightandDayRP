...
local function smashVitrine(k)
    if validWeapon() then
        ...
        elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
            Framework.Functions.Notify("You've left a fingerprint on the glass", "error")
        end
        smashing = true
        Framework.Functions.Progressbar("smash_vitrine", "Looting case", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        Framework.Functions.Notify('Your weapon does not seem strong enough..', 'error')
    end
end
...
local PlantThermite = function()
    ...
    TriggerEvent('inventory:client:ItemBox', Framework.Shared.Items["thermite"], "remove")
    RequestAnimDict("anim@heists@ornate_bank@thermal_charge")
...
RegisterNetEvent('qb-jewellery:client:Thermite', function()
    Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
        if result then 
            ...
            Framework.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                if cops >= Config.RequiredCops then
                    ...
                    function()
                        Framework.Functions.Notify("Thermite failed..", "error")
                    end)
                else
                    Framework.Functions.Notify("Not enough police..", "error")
                end
            end)
        else
            Framework.Functions.Notify("You are missing something(s)..", "error", 2500)
        end
...
CreateThread(function()
    for k, v in pairs(Config.Locations) do
        exports['qb-eye']:AddBoxZone("JewelleryCase"..k, vector3(v.coords.x, v.coords.y, v.coords.z-1), 0.6, 1.2, {
    ...
    exports['qb-eye']:AddBoxZone("JewelleryThermite", vector3(-595.94, -283.74, 50.32), 0.4, 0.8, {
