...
Framework.Functions.CreateCallback('jim-mining:get', function(source, cb, item, tablenumber, craftable)
	local hasitem = false
	local hasanyitem = nil
		for l, b in pairs(craftable[tablenumber][item]) do
			if Framework.Functions.GetPlayer(source).Functions.GetItemByName(l) and Framework.Functions.GetPlayer(source).Functions.GetItemByName(l).amount >= b then hasitem = true
...
RegisterServerEvent('jim-mining:GetItem', function(ItemMake, tablenumber, craftable)
	...
	if craftable then
			...
			for l, b in pairs(craftable[tablenumber][ItemMake]) do
				TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items[tostring(l)], "remove", b) 
	...
	if ItemMake == "copper" or ItemMake == "goldbar" or ItemMake == "iron" or ItemMake == "steel" then
		...
		if breackChance >= 8 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items['drillbit'], 'remove', 1)
		end
	end
	Player.Functions.AddItem(ItemMake, amount, false, {["quality"] = nil})
	TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items[ItemMake], "add", amount)
end)
...
AddEventHandler('jim-mining:MineReward', function()
    ...
    TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items["stone"], "add", randomChance)
end)
...
AddEventHandler('jim-mining:CrackReward', function()
    ...
    TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items["stone"], "remove", 1)
    local oreToGive = nil
    ...
    TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items[Config.RewardPool[oreToGive]], "add", amount)
end)
...
AddEventHandler("jim-mining:Selling", function(data)
    ...
    if Player.Functions.GetItemByName(data) ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, Framework.Shared.Items[data], 'remove', amount)
    else
        TriggerClientEvent("Framework:Notify", src, "You don't have any "..Framework.Shared.Items[data].label, "error")
    end
    Citizen.Wait(1000)
end)
...
Framework.Functions.CreateCallback('jim-mining:Cutting:Check:Tools', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
