...
Citizen.CreateThread(function()
	exports['qb-eye']:AddCircleZone("MineShaft", Config.Locations['Mine'].location, 2.0, { name="MineShaft", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:openShop", icon = "fas fa-certificate", label = "Browse Store", }, },
		distance = 2.0
	})
	exports['qb-eye']:AddCircleZone("Quarry", Config.Locations['Quarry'].location, 2.0, { name="Quarry", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:openShop", icon = "fas fa-certificate", label = "Browse Store", }, },
		distance = 2.0
	})
	--Smelter to turn stone into ore
	exports['qb-eye']:AddCircleZone("Smelter", Config.Locations['Smelter'].location, 3.0, { name="Smelter", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:SmeltMenu", icon = "fas fa-certificate", label = "Use Smelter", }, },
		distance = 10.0
	})
	--Ore Buyer
	exports['qb-eye']:AddCircleZone("Buyer", Config.Locations['Buyer'].location, 2.0, { name="Buyer", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:SellOre", icon = "fas fa-certificate", label = "Sell Ores", },	},
		distance = 2.0
	})
	--Jewel Cutting Bench
	exports['qb-eye']:AddCircleZone("JewelCut", Config.Locations['JewelCut'].location, 2.0, { name="JewelCut", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:JewelCut", icon = "fas fa-certificate", label = "Use Jewel Cutting Bench", },	},
		distance = 2.0
	})
	--Jewel Buyer
	exports['qb-eye']:AddCircleZone("JewelBuyer", Config.Locations['Buyer2'].location, 2.0, { name="JewelBuyer", debugPoly=false, useZ=true, }, 
	{ options = { { event = "jim-mining:JewelSell", icon = "fas fa-certificate", label = "Talk To Jewel Buyer", },	},
		distance = 2.0
	})
	--Cracking Bench
	exports['qb-eye']:AddCircleZone("CrackingBench", Config.Locations['Cracking'].location, 2.0, { name="CrackingBench", debugPoly=false, useZ=true, }, 
	...
	for k,v in pairs(Config.OrePositions) do
		ore = ore+1
		exports['qb-eye']:AddCircleZone(ore, v.coords, 2.0, { name=ore, debugPoly=false, useZ=true, }, 
...
Framework.Functions.TriggerCallback("Framework:HasItem", function(item) 
		if item then 
			...
			Framework.Functions.Progressbar("open_locker_drill", "Drilling Ore..", math.random(10000,15000), false, true, {
			...
			TriggerEvent('Framework:Notify', "You dont have a drill", 'error')
		end 
	end, "drill")
end)
...
AddEventHandler('jim-mining:CrackStart', function ()
	Framework.Functions.TriggerCallback("Framework:HasItem", function(item) 
		if item then 
			...
			Framework.Functions.Progressbar("open_locker_drill", "Cracking Stone..", math.random(10000,15000), false, true, {
				disableMovement = true,	disableCarMovement = true, disableMouse = false, disableCombat = true, }, {}, {}, {}, function() -- Done
			...
			TriggerEvent('Framework:Notify', "You don't have any Stone", 'error')
		end 
	end, "stone")
end)
...
AddEventHandler('jim-mining:MakeItem', function(data)
	for k, v in pairs(data.craftable[data.tablenumber]) do
		Framework.Functions.TriggerCallback('jim-mining:get', function(amount) 
			if not amount then 
				TriggerEvent('Framework:Notify', "You don't have the correct ingredients", 'error')
...
AddEventHandler('jim-mining:MakeItem:Cutting', function(data)
	Framework.Functions.TriggerCallback("jim-mining:Cutting:Check:Tools",function(hasTools)
		if hasTools then
			for k, v in pairs(data.craftable[data.tablenumber]) do
				Framework.Functions.TriggerCallback('jim-mining:get', function(amount) 
					if not amount then 
			...
			TriggerEvent('Framework:Notify', "You don\'t have a Hand Drill or Drill Bit", 'error')
...
function itemProgress(ItemMake, tablenumber, craftable)
	if craftable then
		for i = 1, #Crafting.SmeltMenu do
			for k, v in pairs(Crafting.SmeltMenu[i]) do
				if ItemMake == k then
					bartext = "Smelting "..Framework.Shared.Items[ItemMake].label
					bartime = 7000
		...
		for i = 1, #Crafting.GemCut do
			for k, v in pairs(Crafting.GemCut[i]) do
				if ItemMake == k then
					bartext = "Cutting "..Framework.Shared.Items[ItemMake].label
					bartime = 7000
					animDictNow = "amb@prop_human_parking_meter@male@idle_a"
					animNow = "idle_a"
				end
	...
	Framework.Functions.Progressbar('making_food', bartext, bartime, false, false, {
		...
		TriggerEvent('Framework:Notify', "Cancelled!", 'error')
	end)
end
------------------------------------------------------------
		...
		for i = 1, #Crafting.SmeltMenu do
			for k, v in pairs(Crafting.SmeltMenu[i]) do
				if k ~= "amount" then
					...
					setheader = Framework.Shared.Items[k].label..tostring(amount)
					for l, b in pairs(Crafting.SmeltMenu[i][tostring(k)]) do
						if b == 1 then number = "" else number = " x"..b end
						text = text.."- "..Framework.Shared.Items[l].label..number.."<br>"
						settext = text
					end
					SmeltMenu[#SmeltMenu + 1] = { header = "<img src=nui://"..Config.ImageLink..Framework.Shared.Items[k].image.." width=35px> "..setheader, txt = settext, params = { event = "jim-mining:MakeItem", args = { item = k, tablenumber = i, craftable = Crafting.SmeltMenu } } }
		...
		for i = 1, #Crafting.GemCut do
			for k, v in pairs(Crafting.GemCut[i]) do
				if k ~= "amount" then
					...
					GemCut[#GemCut + 1] = { header = "<img src=nui://"..Config.ImageLink..Framework.Shared.Items[k].image.." width=35px> "..setheader, txt = settext, params = { event = "jim-mining:MakeItem:Cutting", args = { item = k, tablenumber = i, craftable = Crafting.GemCut } } }
		...
		for i = 1, #Crafting.RingCut do
			for k, v in pairs(Crafting.RingCut[i]) do
				if k ~= "amount" then
					...
					RingCut[#RingCut + 1] = { header = "<img src=nui://"..Config.ImageLink..Framework.Shared.Items[k].image.." width=35px>"..setheader, txt = settext, params = { event = "jim-mining:MakeItem:Cutting", args = { item = k, tablenumber = i, craftable = Crafting.RingCut } } }
					settext, setheader = nil
				end
			end
		end
		...
		for i = 1, #Crafting.NeckCut do
			for k, v in pairs(Crafting.NeckCut[i]) do
				if k ~= "amount" then
					...
					NeckCut[#NeckCut + 1] = { header = "<img src=nui://"..Config.ImageLink..Framework.Shared.Items[k].image.." width=35px>"..setheader, txt = settext, params = { event = "jim-mining:MakeItem:Cutting", args = { item = k, tablenumber = i, craftable = Crafting.NeckCut } } }
					settext, setheader = nil
				end
			end
		end
