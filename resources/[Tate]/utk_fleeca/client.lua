...
--https://discord.gg/wQBuB3U5Ym - Dox Shop
--https://discord.gg/wQBuB3U5Ym - Dox Shop
--Made By UTK Converted To Framework By Dox
...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    elseif not oc then
        Framework.Functions.Notify(arg, "error")
    end
end)
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        Framework.Functions.Notify("A bank's alarms are triggered!", 'error', 10000)
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    Framework.Functions.Notify("VAULT DOOR WILL CLOSE IN 30 SECONDS!", 'error')
    Citizen.Wait(30000)
    Framework.Functions.Notify("VAULT DOOR CLOSING!", 'error')
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    Framework.Functions.Notify("Hacking complete!", 'error')
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    Framework.Functions.Notify("You have 2 minutes until the security system activation.")
...
function SecondDoor(data, key)
    ...
    Framework.Functions.Notify("Access complete!", 'success')
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
        ...
        if not done then
            ...
            if dst < 4 then
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    Framework.Functions.TriggerCallback("utk_fh:checkSecond", function(result)
                        if result then
                            done = true
                            return SecondDoor(UTK.Banks[currentname], currentname)
                        elseif not result then
                            Framework.Functions.Notify("You don't have the Secure ID Card.", 'error')
                        end
                    end)
                end
            end
...
Citizen.CreateThread(function()
    ...
    Framework.Functions.TriggerCallback("utk_fh:getBanks", function(bank, door)
        UTK.Banks = bank
        Doors = door
    end)
    --Citizen.Wait(1000)
