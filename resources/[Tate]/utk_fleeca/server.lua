        ...
        if item ~= nil then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent('Framework:Notify', _source, "This bank recently robbed. You need to wait "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent('Framework:Notify', _source, "This bank is currently being robbed.", 'error')
            end
        else
            TriggerClientEvent('Framework:Notify', _source, "You don't have a card.", 'error')
        end
    else
        TriggerClientEvent('Framework:Notify', _source, "There is not enough police in the city.", 'error')
    end
end)
...
Framework.Functions.CreateCallback("utk_fh:getBanks", function(source, cb)
    cb(UTK.Banks, Doors)
end)

Framework.Functions.CreateCallback("utk_fh:checkSecond", function(source, cb)
    local xPlayer = Framework.Functions.GetPlayer(source)
