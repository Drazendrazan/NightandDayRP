...
Citizen.CreateThread(function()
    Wait(500)
    Framework.Functions.TriggerCallback('qb-laundrette:server:GetData', function(data)
                    ...
                    if not washing and not collect then      
                        DrawText3Ds(washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, "~g~E~w~ - Start Washer")
                            if IsControlJustReleased(1, Keys['E']) then
                                WashAnimation()
                                Framework.Functions.Progressbar("bills_wash", "putting inked bills into the washer...", math.random(5000, 10000), false, true, {
            ...
            inRange = true
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, collectCoordX,collectCoordY, collectCoordZ, true) < 0.5 then      
                        if IsControlJustReleased(1, Keys['E']) and inRange then
                            WashAnimation()
                            Framework.Functions.Progressbar("bills_collect", "collecting clean bills from washer...", math.random(10000, 60000), false, true, {
                                disableMovement = true,
                                disableCarMovement = false,
                                disableMouse = false,
                                disableCombat = true,
    ...
    TriggerEvent('qb-laundrette:client:washTimer')
        ...
        while washing do
            ...
            if timer <= 0 then
                washing = false
                collect = true
                Framework.Functions.Notify('Your clean bills are ready to be collected.', 'success', 50000)
    ...
    if ClosestLaundrette == Config.CurrentLaundrette then
        ...
        if dist < 1 then
            if mwkey == ClosestLaundrette then
                ...
                Framework.Functions.Notify('This isn\'t the right key..', 'error')
            end
        end
    end
end)
