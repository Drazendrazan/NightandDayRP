...
Framework.Functions.CreateCallback('qb-laundrette:server:GetData', function(source, cb)
...
Framework.Functions.CreateUseableItem("mwkey", function(source, item)
...
AddEventHandler("qb-laundrette:server:checkInv", function()
    ...
    local Player = Framework.Functions.GetPlayer(src)
        ...
        if Player.Functions.GetItemByName('markedbills') ~= nil then
            ...
            TriggerClientEvent('Framework:Notify', src, 'You put the bills in the washer.', 'success')
            Player.Functions.RemoveItem('markedbills', amt)
        else
            TriggerClientEvent('Framework:Notify', src, 'You have no marked bills.', 'error') 
        end
end)
-- itemData.info.worth
