...
AddEventHandler('Dox-firstdoor', function()
    ...
    if dist < 1.5 then
        Framework.Functions.TriggerCallback('Dox-bobcat:server:getCops', function(cops)
            if cops >= Config.RequiredCops then
                TriggerEvent("Dox-minigamedoor1")
            else
                Framework.Functions.Notify("Not Enough Cops", "error")    
            end
        end)
    end            
end)  
...
function()
    Framework.Functions.Notify("You failed get bettter nub.", "error")
    TriggerServerEvent("Framework:Server:RemoveItem", "thermite", 1)
end)
end)
...
function()
    ...
    Framework.Functions.Notify("Success.", "success")
    TriggerEvent("dispatch:BobCatRobbery")       
    TriggerServerEvent("Framework:Server:RemoveItem", "thermitec", 1)
end,
function()
    Framework.Functions.Notify("You failed get bettter nub.", "error")
    TriggerServerEvent("Framework:Server:RemoveItem", "thermitec", 1)
...
AddEventHandler('Dox-bobcatdooropen', function()
	thermiteanime1()
    TriggerServerEvent('qb-doorlock:server:updateState', 81, false)
end)
...
AddEventHandler('Dox-bobcatseconddoor', function()
	thermiteanime2()
   	TriggerServerEvent('qb-doorlock:server:updateState', 82, false)
end)


RegisterNetEvent('Dox-bobcatthirddoor') -- Third Doorlock 3 
AddEventHandler('Dox-bobcatthirddoor', function()
    TriggerServerEvent('qb-doorlock:server:updateState', 83, false)
    TriggerServerEvent("Framework:Server:RemoveItem", "security_card_01", 1)
...
AddEventHandler('Dox-collect', function()
    if not robberyBusy then
       ...
       Framework.Functions.Notify("Empty.", 'error')
    end
end)
...
AddEventHandler('Dox-pedgoboom', function()
    ...
    exports["qb-eye"]:AddCircleZone("capitulate", vector3(870.4505, -2288.83, 32.441), 1.0, {
        name ="capitulate",
        useZ = true,
        --debugPoly=true
        }, {
        ...
        exports["qb-eye"]:AddCircleZone("lootweapon", vector3(883.0063, -2283.38, 32.441), 1.0, {
            name ="lootweapon",
            useZ = true,
            --debugPoly=true
            }, {
