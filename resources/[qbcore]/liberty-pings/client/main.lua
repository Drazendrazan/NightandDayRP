...
AddEventHandler('qb-pings:client:DoPing', function(id)
    ...
    if not IsHandcuffed then
        ...
        Framework.Functions.Notify('You can\'t ping at the moment.', 'error')
    end
end)
...
AddEventHandler('qb-pings:client:AcceptPing', function(PingData, SenderData)
    ...
    if not IsHandcuffed then
        ...
        Framework.Functions.Notify('You can\'t accept the ping at the moment.', 'error')
    end
end)

RegisterNetEvent('qb-pings:client:SendLocation')
AddEventHandler('qb-pings:client:SendLocation', function(PingData, SenderData)
    Framework.Functions.Notify('The location has been set on your GPS.', 'success')
    ...
    SetTimeout(5 * (60 * 1000), function()
        Framework.Functions.Notify('Ping '..PingData.sender..' Pin has expired..', 'error')
