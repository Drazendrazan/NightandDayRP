...
Citizen.CreateThread(function()
	while true do 
		...
		if swiming and time > 0 then
			...
			DrawText3D(playerpos.x, playerpos.y, playerpos.z, "~r~Your electronic devices will break down due to water in ~w~"..time.."~r~ seconds~w~")
		elseif swiming and time >= -8 then
			local playerpos = GetEntityCoords(PlayerPedId())
			DrawText3D(playerpos.x, playerpos.y, playerpos.z, "~r~Your electronic devices broke down~w~")
		end
	end
end)
